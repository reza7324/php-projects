...
if (isset($_POST['editPost'])) {
    ...
    if (empty(trim($_POST['title']))) {
        $inValidInputTitle = 'فیلد عنوان اجباریست.';
    }
    if (empty(trim($_POST['author']))) {
        $inValidInputAuthor = 'فیلد نویسنده اجباریست.';
    }
    if (empty(trim($_POST['body']))) {
        $inValidInputBody = 'فیلد متن اجباریست.';
    }
    ...
    if (!empty(trim($_POST['title'])) && !empty(trim($_POST['author'])) && !empty(trim($_POST['body']))) {
        ...
        if (!empty(trim($_FILES['image']['name']))) {
            ...
            if (move_uploaded_file($tmpName, "../../../upload/posts/$imageName")) {
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4 mb-5">
            ...
            <div class="mt-4">
                <form method="post" class="row g-4" enctype="multipart/form-data">
                    ...
                    <div class="col-12 col-sm-6 col-md-4">
                        <img class="rounded" src="../../../upload/posts/<?= $post['image'] ?>" width="300" />
                    </div>
